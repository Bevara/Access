...
BOOL Osmo4::InitInstance()
{
	...
	if (!m_user.modules || ! gf_modules_get_count(m_user.modules) ) {
		MessageBox(NULL, "No modules currently available. Contact support@bevaratechnologies.com for assistance.", "Fatal Error", MB_OK);
		m_pMainWnd->PostMessage(WM_CLOSE);
	}
	...
	if (! m_term) {
		MessageBox(NULL, "Cannot load Bevara Access. Contact support@bevaratechnologies.com for assistance.", "Fatal Error", MB_OK);
		m_pMainWnd->PostMessage(WM_CLOSE);
		return TRUE;
	}
	SetOptions();
...
{
	ShellExecute(NULL, "open", "https://www.surveymonkey.com/s/BevaraAccess", NULL, NULL, SW_SHOWNORMAL);
}
...
CString Osmo4::GetFileFilter()
{
	...
	supportedFiles = "All Known Files|*.bvr; *.m3u;*.pls;*.mp4;*.3gp;*.3g2;*.avi;*.ogg;*.mpg;*.mpeg;*.vob;*.vcd;*.svcd;*.ts;*.m2t;*.mp2;*.mp3;*.m1a;*.m2a;*.jpeg;*.jpg;*.j2k;*.jp2";
...
void Osmo4::UpdatePlayButton(Bool force_play)
{
	if (!force_play && gf_term_get_option(m_term, GF_OPT_PLAY_STATE)==GF_STATE_PLAYING) {
		((CMainFrame *) m_pMainWnd)->m_wndToolBar.SetButtonInfo(2, ID_FILE_PLAY, TBBS_BUTTON, 4);
		
	} else {
		((CMainFrame *) m_pMainWnd)->m_wndToolBar.SetButtonInfo(2, ID_FILE_PLAY, TBBS_BUTTON, 3);
