...
void CMainFrame::FileExtract(){
	...
	//in = (LPCTSTR)m_pPlayList->GetURL();
	//mdump.file = gf_isom_open((char*)(LPCTSTR)in, GF_ISOM_OPEN_READ, NULL);
	//if (!mdump.file) return;

	////err = getBevInfo(mdump.file, &version, &comments, &forg, &fext);
	...
	//CString defExt(fext);
	//CString filename;
	//szFilters.Format("%s Files (*.%s)|*.%s|All Files (*.*)|*.*||", ext, ext, ext);
	//filename.Format("%s%s", forg, fext);
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	*ba = ID_EXTRACT_DATA; ba++;
	//*ba = ID_SWITCH_RENDER;
	m_wndToolBar.SetButtons(buttonArray, 7);
	m_wndToolBar.SetButtonInfo(0, ID_OPEN_FILE, TBBS_BUTTON, 0);
	m_wndToolBar.SetButtonInfo(1, ID_SEPARATOR, TBBS_SEPARATOR, 0);
	//m_wndToolBar.SetButtonInfo(2, ID_NAV_PREV, TBBS_DROPDOWN, 1);
	//m_wndToolBar.SetButtonInfo(3, ID_NAV_NEXT, TBBS_DROPDOWN, 2);
	...
	m_wndToolBar.SetButtonInfo(4, ID_SEPARATOR, TBBS_SEPARATOR, 0);
	m_wndToolBar.SetButtonInfo(5, ID_FILE_PROP, TBBS_BUTTON, 7);
	m_wndToolBar.SetButtonInfo(6, ID_EXTRACT_DATA, TBBS_BUTTON, 8);
